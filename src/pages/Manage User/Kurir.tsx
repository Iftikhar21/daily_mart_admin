...
export default function KelolaKurir() {
    const [kurir, setKurir] = useState<Kurir[]>([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const [detailModal, setDetailModal] = useState(false);
    const [currentKurir, setCurrentKurir] = useState<Kurir | null>(null);
    ...
    const fetchKurir = async () => {
        ...
        try {
            ...
            console.error(err);
            setError("Gagal mengambil data kurir");
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredKurir = kurir.filter((k) => {
        ...
        return (
            k.user.name.toLowerCase().includes(s) ||
            k.user.email.toLowerCase().includes(s) ||
            k.no_hp.toLowerCase().includes(s)
        );
    });

    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentItems = filteredKurir.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(filteredKurir.length / itemsPerPage);

    return (
        <Layout title="Kelola Kurir">
            {/* Header */}
            <div className="flex flex-col lg:flex-row justify-between lg:items-center bg-white rounded-xl shadow-sm p-6 mb-6 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Kelola Kurir</h1>
                    <p className="text-gray-600">Daftar Kurir (Read Only)</p>
                    {error && <p className="text-red-500 mt-1">{error}</p>}
                </div>

                <div className="text-gray-700 text-sm lg:text-base flex flex-col text-left lg:text-right">
                    <span className="font-medium text-2xl text-blue-700">{kurir.length}</span>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-2 md:space-y-0 md:space-x-4">
                    ...
                    <div className="text-sm text-gray-500 md:ml-auto">
                        Menampilkan {currentItems.length} dari {filteredKurir.length} kurir
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <h3 className="mt-4 text-lg font-medium text-gray-900">Tidak ada kurir</h3>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {currentItems.map((k) => (
                                        <tr key={k.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 whitespace-nowrap">#{k.id}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{k.user.name}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{k.user.email}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{k.no_hp || "-"}</td>
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-between mt-6">
                                <div className="text-sm text-gray-700">
                                    ...
                                    {Math.min(indexOfLastItem, filteredKurir.length)} dari{" "}
                                    {filteredKurir.length}
                                </div>
            ...
            {/* Modal Detail */}
            <Modal isOpen={detailModal} title="Detail Kurir" onClose={() => setDetailModal(false)}>
                {currentKurir && (
                    <div className="space-y-6 p-1">
                        <div className="flex items-center space-x-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                            ...
                            <div>
                                <h3 className="font-semibold text-lg text-gray-800">{currentKurir.user.name}</h3>
                                <p className="text-gray-600">{currentKurir.user.email}</p>
                            </div>
                            <div className="ml-auto">
                                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-yellow-100 text-yellow-800">
                                    {currentKurir.user.role}
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {currentKurir.no_hp && (
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium text-gray-800">{currentKurir.no_hp}</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-800">#{currentKurir.user_id}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-800">
                                            {new Date(currentKurir.user.created_at).toLocaleString('id-ID')}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-800">
                                            {new Date(currentKurir.user.updated_at).toLocaleString('id-ID')}
                                        </p>
                                    </div>
                                </div>
                            </div>
